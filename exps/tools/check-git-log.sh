...
elif printf -- "%s" "$range" | grep -q '^-[0-9]\+' ; then
	range="HEAD$(printf -- "%s" "$range" | sed 's,^-,~,').."
else
...
# check tag order
bad=$(echo "$tags" | awk \
	'BEGIN {reported=0; suggested=0; signed=0; reviewed=0; fixes=0;}
	/Fixes:/{fixes=1; if(signed || reviewed || suggested || reported) print "Fixes: tag in wrong position."}
	/Reported-by:/{reported=1; if(suggested || reviewed || signed) print "Reported-by: tag in wrong position"}
	/Suggested-by:/{suggested=1; if(reviewed || signed) print "Suggested-by: tag in wrong position"}
	/Signed-off-by:/{signed=1; if(reviewed) print "Signed-off-by: tag in wrong position"}
	/Reviewed-by:/{reviewed=1}
	')
[ -z "$bad" ] || { printf "Wrong tag order:\n%s\n" "$bad" && failure=true;}

total=$(echo "$commits" | wc -l)
if $failure ; then
	printf "\nInvalid patch(es) found - checked %s patch" "$total"
else
	printf "\n%s/%s valid patch" "$total" "$total"
fi
